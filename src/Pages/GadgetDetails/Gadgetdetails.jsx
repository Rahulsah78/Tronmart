...
const Gadgetdetails = () => {
    ...
    const gadgetproduct = gadgetdata.find((item) => item.id === parseInt(gadgetId));
    ...
    const productInCart = cart.find((item) => item.id === gadgetproduct.id);
    ...
    const handleSearchClick = () => {
        setGalleryImages([{ largeURL: gadgetproduct.img, thumbnailURL: gadgetproduct.img }]);
    ...
    return (
        <Layout>
            <div className="p-6 sm:p-10 bg-[#F7FAFD] min-h-screen flex flex-col md:flex-row">
                ...
                >
                    <img
                        src={gadgetproduct.img}
                        alt={gadgetproduct.name}
                ...
                <div className="w-full md:w-[60%]  sm:p-10">
                    <h2 className=" sm:text-2xl font-bold text-gray-800">{gadgetproduct.name}</h2>
                    <p className="text-gray-600 mt-2">{gadgetproduct.description}</p>

                    {/* Price */}
                    <div className="flex gap-3 items-center pt-5">
                        <span className="font-semibold text-base sm:text-lg">
                            Rs {(gadgetproduct.price - (gadgetproduct.price * gadgetproduct.discount) / 100).toFixed()}
                        </span>
                        <del className="text-gray-600">Rs {gadgetproduct.price}</del>
                        <span className="text-red-600 text-sm sm:text-base">
                            ({gadgetproduct.discount}% OFF)
                        </span>
                    </div>
                    ...
                    <div className="mt-6">
                        ...
                        <ul className="list-disc pl-8 text-sm sm:text-base text-gray-700">
                            {gadgetproduct.keyFeatures?.map((feature, index) => (
                    ...
                    <div className="flex flex-wrap gap-5 items-center mt-6">
                        ...
                        <button
                            onClick={() => ADDTOCART(gadgetproduct)}
